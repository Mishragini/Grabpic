...
from lib.utils  import generate_invite_code,check_event,delete_bucket_folder,success_response_handler
...
async def create_space(request:Request,space:Annotated[CreateSpaceRequest,Body()]):
    ...
    supabase_res = await asyncio.to_thread(supabase.table("events").insert({
        "name":space.name,
    ...
    return success_response_handler(message="Event created successfully",data={"event":event_data})
...
async def fetch_spaces(request:Request,page:Annotated[int,Query()]=0,per_page:Annotated[int,Query()]=5):
    ...
    space_db_res = await asyncio.to_thread(supabase.table("events")\
    ...
    return success_response_handler(message="Events fetched successfully",data={"events":space_data})        
...
async def fetch_space(req:Request,event_id:str):
    ...
    db_res = await asyncio.to_thread(supabase.table("events")\
    ...
    return success_response_handler(message=f"Event with {event_id} fetched successfully!",data={"event":event})
...
async def delete_event(req:Request,event_id:str):
    ...
    await asyncio.to_thread(supabase.table("events").delete().eq("id",event_id).execute)